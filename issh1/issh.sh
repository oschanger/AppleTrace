...
function checkIproxy(){
    if hash iproxy 2>/dev/null; then 
        iSSHDLOG "iproxy install. lets go"
    else 
        iSSHELOG "iproxy not install. try \"brew install usbmuxd\""
        return 1
    fi

    ret=`lsof -i tcp:2222 | grep "iproxy" | head -1`
    if [[ "$?" = "0" ]]; then
        iproxyPid=`echo $ret | awk '{print $2}'`
        iSSHDLOG "iproxy process for 2222 port alive, pid=$iproxyPid"
    else
...
function printUsage(){
    ...
    printf "issh %-30s %-20s \n" "setpid [pid1,pid2]" "set pids to trace" 
    printf "issh %-30s %-20s \n" "setname [procname1,procname2]" "set procs to trace" 
    printf "issh %-30s %-20s \n" "trace [-t time]" "start trace, default 5s" 
    ...
    printf "issh %-30s %-20s \n" "install" "install app from local to connect device"
...
function issh(){    
    ...
    if [[ "$1" = "setname" ]]; then
        names=`echo $2 | awk '{split($0,arr,",");for(i in arr) print arr[i]}'`
        echo $names
        targetFile="/var/mobile/Library/AppleTraceTargets"
        sshRunCMDClean "echo '$names' > $targetFile"
    fi

    if [[ "$1" = "setpid" ]]; then
        targetFile="/var/mobile/Library/AppleTraceTargets"
        appleTraceFramework="/usr/lib/TweakInject/appletrace.framework/appletrace"
        PSOUT=$(sshRunCMDClean "ps -e  | grep -v grep" )
        sshRunCMD "rm $targetFile"

        #start="cynject $2 /usr/lib/TweakInject/appletrace.framework/appletrace"

        #1. issh trace pid1,pid2,pid3
        pids=`echo $2 | awk '{split($0,arr,",");for(i in arr) print arr[i]}'`
        pidarr=()
        procnamearr=()
        echo $pids | while read line 
        do
            pid=$line
            appProcessName=`echo $PSOUT | grep $pid |head -1| awk -F '[ /]+' '{print $NF}'`
            echo $pid,$appProcessName
            #zsh array starts from 1, not 0
            pidarr[${#pidarr[*]}+1]=$pid
            procnamearr[${#procnamearr[*]}+1]=$appProcessName
        done
        for i in $procnamearr
        do
            sshRunCMDClean "echo $i >> $targetFile"
        done
        for i in $pidarr
        do
            sshRunCMD "cynject $i $appleTraceFramework"
            #sshRunCMDClean "kill -9 $i"
        done      
        iSSHILOG "Kill $procnamearr,Please restart them"
    fi

    if [[ "$1" = "trace" ]]; then
        traceFolder="/var/mobile/Library/AppleTrace/*"
        sshRunCMD "rm -rf $traceFolder"
        sshRunCMD "trace start"
        sleepTime=5
        iSSHILOG "sleep $sleepTime"
        sleep $sleepTime
        
        sshRunCMD "trace stop"
        sleep 1
        iSSHILOG "pull trace $traceFolder (require rsync in cydia)"
        time=$(date "+%Y-%m-%d_%H-%M-%S")
        mkdir $time
        #scp will "Broken Pipe"
        #scp -P 2222 -r root@localhost:$traceFolder $time
        rsync -avz -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p 2222" --progress -r root@localhost:$traceFolder $time
        outTracePath="/Users/leo/Documents/AppleTrace"
        iSSHILOG "merge result"
        python $outTracePath/merge.py -d $time
    fi
    ...
    if [ "$1" = "show" ];then
        case $2 in
            ...
            trace* )
                sshRunCMD "cat /var/mobile/Library/AppleTraceTargets"
                ;;
...
EOF'
    ...
    if [[ "$1" =~ "app" ]]; then
        ...
        do
            ...
            process=( $line )
            appPid=`echo $process | awk -F '[ /]+' '{print $2}'`
            appPath=${process[7]}
            echo $appPid
            x=`echo $process | awk -F '[ /]+' '{print $NF}'`
            echo $x
